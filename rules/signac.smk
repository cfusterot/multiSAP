...
if config["signac"]["enable"]:
    ...
    rule step3_signac:
        input:
            seurat_in = expand("{OUTDIR}/signac/SeuratObject_{alias}.rds", OUTDIR=OUTDIR, alias=samples.loc[wc.sample]['alias'])
        output:
    ...
    rule step4_signac:
        input:
            seurat_out = expand("{OUTDIR}/signac/SeuratObject_{condition}.rds", OUTDIR=OUTDIR, condition=samples.loc[wc.sample]['condition'])
        output:
