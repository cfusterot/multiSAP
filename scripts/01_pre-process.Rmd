...
# p1 <- LabelClusters(p, id = "colors")
# 
# p2 <- pbmc@meta.data %>%
#   group_by(orig.ident, predicted.celltype.l2) %>%
#   summarize(count = n()) %>% 
#   ggplot(aes(x = orig.ident, y = count)) +
#   geom_bar(aes(fill = predicted.celltype.l2), position = 'fill', stat = 'identity', width=0.01) +
...
## Scrubelt
  # singlets <- data.table::fread(paste0("../../data/", params$sample, "/scrublet.tsv")) %>%
  #   data.frame() %>% dplyr::filter(!called) %>% pull(barcode)
  # singlets.metadata <- rep("multiplet", length(Cells(pbmc)))
  # names(singlets.metadata) <- Cells(pbmc)
  # singlets.metadata[singlets] <- "singlet"
  # pbmc$multiplet_scrubelt <- singlets.metadata
...
```{r, include=FALSE}
# save RDS file
saveRDS(pbmc, file.path(directory, "signac", paste0(Sys.Date(), "_", params$sample, "_Seurat.rds")))

celltype <- data.frame(predicted.celltype.l2 = pbmc$predicted.celltype.l2,
           refUMAP_1 = pbmc$refUMAP_1,  refUMAP_2 = pbmc$refUMAP_2)

write.csv(celltype, file.path(directory, "signac", paste0(Sys.Date(),"_", params$sample, "_pred.celltype.l2.csv" ))
```
...
homoVar <- mtVAR.meta %>% filter(vmr < 0.01) %>% arrange(position) %>% pull(variant)

M.VAR <- c("m.489T>C", "15043G>A", "10400C>T", "14783T>C", "15301G>A")
...
Macro_haplogroup <- ifelse(sum(homoVar %in% M.VAR) > 2, "M",
                    ifelse(sum(homoVar %in% L.VAR) > 2, "L", 
                    ifelse(sum(homoVar %in% N.VAR) > 2,  "N", "Wrong")))
